...
class WordIllustration extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      height: size,
      child: CustomPaint(
        painter: _SoftScenePainter(colors: colors, emoji: emoji),
      ),
    );
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (emoji != null && emoji!.isNotEmpty) {
      final tp = TextPainter(
        text: TextSpan(
          text: emoji,
          style: TextStyle(fontSize: size.width * 0.42),
        ),
...
class _FoxPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final dx in [-0.12, 0.12]) {
      canvas.drawCircle(
        Offset(cx + size.width * dx, cy - size.height * 0.04),
        size.width * 0.07,
        Paint()..color = Colors.white,
      );
      canvas.drawCircle(
        Offset(cx + size.width * dx, cy - size.height * 0.02),
        size.width * 0.04,
    ...
    if (emotion == FoxEmotion.celebrate) {
      canvas.drawCircle(
        ...
        4,
        Paint()..color = NokhchiinColors.warning,
      );
      canvas.drawCircle(
        Offset(cx + size.width * 0.38, cy - size.height * 0.3),
        3,
        Paint()..color = NokhchiinColors.meadow,
      );
    }
  }
